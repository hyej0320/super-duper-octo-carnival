...
public class C06_Extend {
	...
	/*
		...
		- 자식 클래스는 부모로부터 물려받은 변수 및 기능을 
	 	...
	 	- 자식 클래스에는 부모 클래스에 없는 기능을 새로 추가해서 사용할 수 있다
	 	- 상속 시 자식 클래스에서는 반드시 부모 클래스의 생성자를 가장 먼저 호출해야 한다 
	 	
	 	# super
	 	
	 	- 자식 클래스로 생성된 인스턴스의 부모 부분을 의미한다
	 	- 자식 클래스에서 this는 자식 클래스 자신만을 의미한다
	 	- 자식 클래스와 부모 클래스에서 같은 이름을 가지고 있는 자원을
	 	  구분하기 위해서 사용한다 
	...
	public static void main(String[] args) {
		...
		Starbucks s1 = new Starbucks("노원점");
		Starbucks s2 = new Starbucks("용산점");
		...
		PaulBassett p1 = new PaulBassett();
...
class Starbucks {
	...
	public Starbucks(String storeName) {
		this.storeName = storeName;
	}
	
	void menuQty() {
		System.out.printf("스타벅스 %s의 메뉴는 총 %d개 입니다.\n", storeName, menuQty);
	}
			
	void storeSize(int width, int height) {
		System.out.printf("스타벅스 %s의 크기는 %d * %d 입니다\n", storeName, width, height);
	}
	
	void seatQty(int qty) {
		System.out.printf("스타벅스 %s의 좌석 수는 %d석 입니다.\n", storeName, seatQty);
	}
	
}

class PaulBassett extends Starbucks {
...
class PurchaseBuilding extends Building {
	...
	// 하지만, 기본 생성자는 생략이 가능하기 때문에 작성하지 않는 것처럼 보이는 경우도 있다
