...
public class D13_DateTime {

	/*
		# java.time 패키지
		
		- Calendar의 여러가지 단점을 보완한 패키지
		- Calendar가 set(), add()를 통해 원본 인스턴스를 변경하는 것이 큰 단점이라고 생각하여
		  새로운 시간 인스턴스를 만들어 반환하는 방식을 사용한다
	...
	public static void main(String[] args) {
		...
		// plus, minus 메서드를 통해 해당 인스턴스를 기준으로 새로운 시간을 만들어 낼 수 있다
		// 캘린더처럼 원본을 훼손하는 것이 아니라 새로운 인스턴스를 만들어낸다
		System.out.println(now_dt.plusDays(19));
		...
		System.out.println("오늘 요일? " + now_d.getDayOfWeek());
		...
		System.out.println("달이름: " + thisMonth.getDisplayName(TextStyle.FULL, Locale.GERMANY));
		
		
		// of(): 원하는 시간의 인스턴스를 생성하는 스태틱 메서드
		LocalDate date1 = LocalDate.of(2001, 5, 13);
		LocalTime time1 = LocalTime.of(14, 35);
		...
		LocalDateTime ukdt = LocalDateTime.now(ZoneId.of("Europe/London"));
		System.out.println(ukdt);
...
//		}
		...
		System.out.println("어린이날 다음달의 마지막 날: " 
				+ childrensDay.plusMonths(1).with(TemporalAdjusters.lastDayOfMonth()));
		
		System.out.println("저번주 수요일 인스턴스: " 
				+ LocalDate.now().with(TemporalAdjusters.previous(DayOfWeek.WEDNESDAY)));
		
		System.out.println("이번달의 두번째 목요일: " 
