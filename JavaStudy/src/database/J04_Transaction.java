...
public class J04_Transaction {
	...
	public static void main(String[] args) {
		...
		try (
			...
			boolean saleAlreadyExists = false;
			try(
				PreparedStatement pstmt = conn.prepareStatement(sql1);
				ResultSet rs = pstmt.executeQuery();
			){
				rs.next();
				saleAlreadyExists = rs.getInt(1) > 0;
			...
			try(
				// 매출이 존재하면 update, 존재하지 않으면 insert
				PreparedStatement pstmt = conn.prepareStatement(saleAlreadyExists ? update : insert);
			){ 
				pstmt.setInt(1, 3000); // ?에 들어갈 데이터 입력
			...
			// 2. 해당 과일의 재고를 감소시킨다
			String reduceQty = "UPDATE fruits SET qty = qty - ? WHERE fruit_name = ?";
			try (
				PreparedStatement pstmt = conn.prepareStatement(reduceQty);
			){
				pstmt.setInt(1, 1); // 개수 감소
			...
			// 자동 커밋이 꺼져있기 때문에 제대로 모두 수행했다면 수동으로 커밋해줘야 한다.
			conn.commit();
		} catch (SQLException e) {
			e.printStackTrace();
		}
