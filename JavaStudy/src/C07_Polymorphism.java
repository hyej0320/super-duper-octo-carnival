...
public class C07_Polymorphism {
	...
	/*
		...
		- ex) 강아지는 강아지이기도 하고 동물이기도 하다
			  (강아지는 동물이다(O), 동물은 강아지다(X))
		- ex) 아메리카노는 아메리카노이기도 하고 커피이기도 하다
	 	...
	 	- 업 캐스팅된 상태에서는 자식 클래스였을 때 가지고 있던 기능은 사용할 수 없다
	 	- 자식 클래스 인스턴스가 업 캐스팅된 상태에서 
	 	  오버라이딩 된 메서드(자식 클래스에서 마음대로 개조한 메서드)를 사용하는 경우 
	 	  자식 클래스의 메서드가 실행된다
	...
	public static void main(String[] args) {
		...
		Dog dog1 = new Dog();
		Wolf dog2 = new Dog("잠시 늑대였던 개");
		
		dog1.walk();
		//dog2.walk(); // 업 캐스팅 되어서 사용 불가능
		
		dog1.pet();
		dog2.pet(); // 오버라이딩 된 메서드는 업 캐스팅 되더라도 자식 클래스의 것으로 동작한다 
		
		dog1.run();
		dog2.run();
		
		// Wolf타입이 되었던 Dog인스턴스를 다시 Dog으로 변환하는 것은 가능하다
		Dog d3 = (Dog)dog2;
		d3.walk();
		
		// 업 캐스팅 된 상태로도 해당 클래스의 인스턴스인지 검사가 가능하다
		Wolf wolf1 = new Dog();
		Wolf wolf2 = new Wolf();
