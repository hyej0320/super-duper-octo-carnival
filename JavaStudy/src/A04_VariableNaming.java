...
public class A04_VariableNaming {
	...
	/*
	 	...
	 	1. 코드의 가독성을 높일 수 있다
	 	2. 여러 곳에서 사용되는 값을 한번에 변경할 수 있다 (값을 재사용하기 편해진다)
	 
	 	# 변수의 작명 규칙 (권장사항)
	 	
	 	1. 두 단어 이상을 이어붙여 사용할 때 _underbar를 사용한다
	 	   (C언어 스타일, scake_case)
	 	...
	 	4. 프로그램이 종료될 때까지 값이 절대로 변하지 않을 값(상수)은 모두 대문자를 사용한다
	 	...
	 	6. 해당 변수에 들어있는 값이 무엇인지 쉽게 파악할 수 있도록 이름을 사용한다
		...
		4. 이미 Java에서 사용하고 있는 키워드들은 변수명으로 사용할 수 없다
		   (Eclipse에서 보라색으로 표시되는 키워드들)
		   ex) public, static, void, int, double, ... (x)
		   
		   
		다음 중 변수의 이름으로 사용할 수 있는 것은? (모두 고르시오) : a,d,e,g
		a. $ystem
		b. channel#5
		c. 7eleven
		d. If
