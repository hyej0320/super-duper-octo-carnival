...
public class D11_Date {
	...
	public static void main(String[] args) {
		...
		// System.currnetTimeMillis() : 현재 시간의 유닉스 타임을 long 타입으로 반환한다
		// ※ Unix Time - 1970년 1월 1일 오전 9시 이후로 시간이 얼마나 흘렀는 지 밀리초(1/1000)로 센 것
		System.out.println("1970년 이후로 흐른 시간: " + System.currentTimeMillis() / 1000 + "초");
		
		
		// 원하는 날짜로 시간을 생성하는 기능이 있지만 더 이상의 업데이트를 해주지 않는다(지원 중단)
		Date d3 = new Date(1995, 3, 20);
		...
		// : 두 시점을 비교하여 어떤 시점이 더 뒤인지 알 수 있다
		System.out.println("now가 onceUponATime보다 이후의 시점이니? " +now.after(onceUponATime));
		System.out.println("now가 onceUponATime보다 이전의 시점이니? " +now.before(onceUponATime));
		...
		// 다양한 getter들이 존재하지만 모두 지원이 중단되어서 별로 사용되지 않는다
