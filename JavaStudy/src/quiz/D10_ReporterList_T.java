...
public class D10_ReporterList_T {
	...
	public static void main(String[] args) {
		...
		while(m.find()) {
			...
			String email = m.group(4);
			String field = m.group(2);
			
			reporters.add(new Reporter(m.group(3), m.group(1), m.group(2), m.group(4)));
			
			
			// getOrDefault(key, default) : 해당 key값으로 get이 안되면 기본값을 준다
...
//			}	
		...
		System.out.println("-------------------- 신문사별 분류 ---------------------");
		
		
		// Map: 키와 값을 저장할 수 있는 것
		// categoryCompany : 키 값으로 회사의 이름을 쓰고 값으로 리스트를 사용한다 
		...
		for (Reporter reporter : reporters) {
			// 기자를 한 명씩 꺼내면서 해당 기자의 회사 이름을 꺼낸다
			String companyName = reporter.getCompany();
			

			if(categoryCompany.containsKey(companyName)) {
				// add(reporter)는 새 인스턴스가 생기는 것이 아니라 for문에 report를 넣는 것이다
				categoryCompany.get(companyName).add(reporter);
			} else {
				// if가 아니면 list를 생성해서 reporter를 add해주고 categoryCompany에 put을 해준다
