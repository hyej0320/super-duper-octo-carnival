...
public class D10_ReporterList {
	...
	/*
	 	1. 어떤 이메일 도메인이 몇 개씩 있는지 세어서 출력해보기
	...
	public static void main(String[] args) {
		Pattern domain = Pattern.compile("(\\w)(@\\w+\\.\\w+(\\.\\w+)?)");
		Matcher domainMatcher = domain.matcher(reporters);

		HashMap<String, Integer> domainCount = new HashMap<>();
		
		while (domainMatcher.find()) {
			System.out.println(domainMatcher.group(2));
