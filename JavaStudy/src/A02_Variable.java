...
public class A02_Variable {
	...
	/*
	 ...
	 - 변수 선언을 통해 메모리에 공관을 확보한 후 데이터를 해당 메모리에 저장해 둘 수 있다
	 ...
	 - 주로 어떤 프로그램으로 '저장하기' 기능을 수행했을 때 보조기억장치에 해당 데이터를 보관한다
	 
	 하드드라이브(느리다):냉동고, 메모리:도마 같은 것
	 하드드라이브에서 프로그램을 실행할 때 메모리(RAM)에서 CPU로 보낸다. 즉, RAM이 가득차면 속도가 느려진다
	 SSD는 전기적인 신호로 되어있기 때문에 빠름: 하이브리드 같은?
	...
	public static void main(String[] args) {
		...
		System.out.println("사용자 점수는 " + user_score + "이고, 컴퓨터 점수는" + com_score + "점입니다.");
		System.out.println("현재 설정된 환영 인사말: " + welcome);
		System.out.println("현재 설정된 퇴장 인사말: " + goodbye);
	
		// (4) 한번에 변수 여러 개 선언과 동시에 대입
