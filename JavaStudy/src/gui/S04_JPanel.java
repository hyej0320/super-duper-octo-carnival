...
public class S04_JPanel {
	/*
	 	...
	 	- 컨테이너 역할을 수행하는 컴포넌트
	 	- 레이아웃을 설정할 수 있기 때문에 여러가지 레이아웃을 함게 적용할 수 있다
	*/
	
	public static void main(String[] args) {
		JFrame f = new JFrame();
		
		// JPanel은 프레임 위에 붙일 수 있는 컴포넌트 이지만 컨테이너 역할을 한다
		...
		ActionListener btnEventListener = new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				// 이벤트 발생할때마다 e에 이벤트 정보가 함께 도착한다
				
				// 어떤 버튼이 이 이벤트를 발생 시켰는지 getSource()를 통해 알 수 있따
				JButton btn = (JButton)e.getSource();
				System.out.println(btn.getText() + "이 눌렸다");
