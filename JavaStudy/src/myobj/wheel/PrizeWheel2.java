...
public class PrizeWheel2 {
	...
	// 총 각도가 360도가 넘는 지 확인하기 위해 필요한 메서드
	...
	public void add(Prize prize) {
		if (prize.getDegree() < 0) {
			System.out.printf("추가하려는 상품 \"%s\"의 각도가 음수입니다", prize);
			return;
		}
	...
	public void modifyDegree(int prizeIndex, int degree) {
		...
		if (degree < 0) {
			System.out.println("[ERROR] 수정하려는 상품의 각도가 올바르지 않습니다. ");
			return;
		} else if (getTotalDegree() - toModify.getDegree() + degree > 360) {
			System.out.printf("[ERROR] \"%s\"의 각도를 %d도에서 %d도로 수정할 수 없습니다. "
					+ "(수정 시 360도 초과)\n", 
