...
public class E00_Exception {

	/*
		# 예외 (Exception)
		
		- 우리가 에러라고 생각했던 빨간 글씨들을 예외라고 부른다
		- 프로그래머는 발생할 수 있는 예외를 미리 예상하고 대비해 둘 수 있다 (예외 처리)
		...
		- 예외가 발생했을 때 예외처리가 되어있지 않은 경우 프로그램이 강제 종료되지만 
		  해당 예외에 대한 처리가 되어있는 경우 프로그램이 강제 종료되지 않는다
	...
	public static void main(String[] args) {
		try {
			int size = 10; // 발생 가능한 예외1
			int[] arr = new int[size];
			System.out.println(arr[-1]); // 발생 가능한 예외2
		} catch (java.lang.ArrayIndexOutOfBoundsException e) {
			System.out.println("인덱스가 잘못된 숫자입니다");
		} catch (java.lang.ArithmeticException e) {
			// 예외가 발생했을 때 당시의 스택 상황을 출력해주는 메서드 (예외 처리 기본 동작1)
			e.printStackTrace();
			// 프로그램을 강제로 종료한다 (예외 처리 기본 동작2) - 예외 처리를 하지 않은 것과 같다
			System.exit(-1); // -1는 비정상 종료의 의미
		}
		// 스택: 프로그램에서 함수 호출 순서를 기억하기 위한 자료구조 (First In Last Out)
		try {
		...
		try {
			...
			// Exception은 모든 예외객체의 조상 클래스이기때문에
