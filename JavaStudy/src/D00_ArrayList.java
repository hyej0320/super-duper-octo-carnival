...
public class D00_ArrayList {
	/*
		...
		- 자료 구조들을 구현해 놓은 클래스들의 모음을 자바에서는 컬렉션이라고 부른다
	...
	public static void main(String[] args) {
		...
		// 어떤 타입으로 꺼내야 할 지 모르는 경우 Object타입으로 꺼낸다
		...
		System.out.println((int)list.get(5) + (double)list.get(6));
		...
		// list.remove(index) : 해당 번째 값을 삭제한다 (삭제에 성공한 값을 변환한다)
		...
		//						일치하는 값이 여러 개면 맨 앞에 있는 값 하나만 지운다 (성공 여부를 반환한다)
		// ※ int의 경우 값으로 삭제하고 싶으면 Integer.valueOf() 메서드를 사용해야 한다
		boolean result = scores.remove(Integer.valueOf(88));
		System.out.println("삭제 성공 여부: " + result);
		System.out.println(scores);
		
		// list.contains(value) : 해당 값의 포함 여부를 반환한다
		System.out.println("30점이 있습니까? " + scores.contains(30));
