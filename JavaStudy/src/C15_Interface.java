...
public class C15_Interface {
	/*
	 	...
	 	- 인터페이스 내부에 선언하는 메서드는 모두 자동으로 public abstract이 된다
	 	- 인터페이스 내부에 선언하는 변수들은 자동으로 final static이 된다
	 	- 인터페이스는 인스턴스화 하는 것이 불가능하다
		...
		- 추상 클래스는 하나밖에 상속받지 못하지만 인터페이스는 여러 개 구현받을 수 있다
	 */
...
interface Swimmer {
	void swim(); // interface 내부의 메서드는 자동적으로 public abstract이 된다
}
